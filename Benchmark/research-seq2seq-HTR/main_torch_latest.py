...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "start_epoch",
        type=int,
        help="load saved weights from which epoch, -1 loads the latest model",
    )
    parser.add_argument("save_weights", type=str, help="location for saving/loading weights")
    parser.add_argument("data_location_train", type=str, help="location for dataset used")
    args = parser.parse_args()
    set_dataset_location(args.data_location_train, args.data_location_train)
    save_weights = args.save_weights
    # prevent clash of different log-directories
...
def loss_label_smoothing(predict, gt):
    def smoothlabel_torch(x, amount=0.25, variance=5):
        ...
        smoothed = (
            x * torch.from_numpy(1 - noise.sum(1)).view(-1, 1).cuda()
...
def teacher_force_func(epoch):
    if epoch < 50:
        ...
        teacher_rate = (50 - (epoch - 50) // 2) / 100.0
    else:
        teacher_rate = 0.0
    return teacher_rate
...
def teacher_force_func_2(epoch):
    if epoch < 200:
        teacher_rate = (100 - epoch // 2) / 100.0
    else:
        teacher_rate = 0.0
    return teacher_rate
...
def train(train_loader, seq2seq, opt, teacher_rate, epoch):
    ...
    for num, (train_index, train_in, train_in_len, train_out) in enumerate(train_loader):
        ...
        output, attn_weights = seq2seq(
            train_in, train_out, train_in_len, teacher_rate=teacher_rate, train=True
        )  # (100-1, 32, 62+1)
        ...
        if LABEL_SMOOTH:
            ...
            loss = F.cross_entropy(
                output_l.view(-1, vocab_size), train_label, ignore_index=tokens["PAD_TOKEN"]
            )
        opt.zero_grad()
        loss.backward()
        opt.step()
...
def valid(valid_loader, seq2seq, epoch):
    ...
    for num, (test_index, test_in, test_in_len, test_out) in enumerate(valid_loader):
        with torch.no_grad():
            ...
            test_in, test_out = (
                Variable(test_in, volatile=True).cuda(),
                Variable(test_out, volatile=True).cuda(),
            )
            output_t, attn_weights_t = seq2seq(
            ...
            if LABEL_SMOOTH:
                ...
                loss_t = F.cross_entropy(
                    output_t.view(-1, vocab_size), test_label, ignore_index=tokens["PAD_TOKEN"]
                )
...
def test(test_loader, modelID, showAttn=True):
    ...
    decoder = Decoder(
        HIDDEN_SIZE_DEC, EMBEDDING_SIZE, vocab_size, Attention, TRADEOFF_CONTEXT_EMBED
    ).cuda()
    seq2seq = Seq2Seq(encoder, decoder, output_max_len, vocab_size).cuda()
    model_file = f"{save_weights}/seq2seq-" + str(modelID) + ".model"
...
def main(train_loader, valid_loader, test_loader):
    ...
    seq2seq = Seq2Seq(encoder, decoder, output_max_len, vocab_size).cuda()
    global CurriculumModelID
    if CurriculumModelID == -1:
        # recover the last state or intialize new model using ID=0
        if not os.path.exists(save_weights):
            os.makedirs(save_weights)
        CurriculumModelID = max(
            [int(x.split("-")[1].split(".")[0]) for x in os.listdir(save_weights)] + [0]
        )
    if CurriculumModelID > 0:
        model_file = f"{save_weights}/seq2seq-" + str(CurriculumModelID) + ".model"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(
            "epoch %d/%d, loss=%.3f, lr=%.8f, teacher_rate=%.3f, time=%.3f"
        ...
        if EARLY_STOP_EPOCH is not None:
            gt = "dataset/RWTH.iam_word_gt_final.ascii.valid.thresh"
            decoded = f"pred_{save_weights}_logs/valid_predict_seq." + str(epoch) + ".log"
            res_cer = sub.Popen(["./tasas_cer.sh", gt, decoded], stdout=sub.PIPE)
            res_cer = res_cer.stdout.read().decode("utf8")
            print("res_cer", res_cer)
...
if __name__ == "__main__":
    ...
    os.system("./test.sh " + tr(f"pred_{save_weights}_logs/test_predict_seq.{mejorModelID}.log"))
    print(time.ctime())
