#!/bin/bash
#SBATCH --job-name=HTR_Baseline_Generation
#SBATCH --ntasks=1
...
# This training run uses the actual GANwriting-baseline dataset to train
if [ ! -d Benchmark/research-seq2seq-HTR/dataset/IAM_baseline ]; then
  python3 generatePseudoIAM.py save_weights Benchmark/research-seq2seq-HTR/dataset/IAM_baseline
fi
cd Benchmark/research-seq2seq-HTR/
python3 main_torch_latest.py -1 HTR_Baseline_weights IAM_baseline
