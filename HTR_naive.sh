#!/bin/bash
#SBATCH --job-name=HTR_naive_Generation
#SBATCH --ntasks=1
...
# This training run uses the naive-patch-loss dataset to train
if [ ! -d Benchmark/research-seq2seq-HTR/dataset/IAM_naive ]; then
  python3 generatePseudoIAM.py save_weights_naive Benchmark/research-seq2seq-HTR/dataset/IAM_naive --patch_loss
fi
cd Benchmark/research-seq2seq-HTR/
python3 main_torch_latest.py -1 HTR_naive_weights IAM_naive
