#!/bin/bash
#SBATCH --job-name=HTR_smart_Generation
#SBATCH --ntasks=1
...
# This training run uses the smart-patch-loss dataset to train
if [ ! -d Benchmark/research-seq2seq-HTR/dataset/IAM_smart ]; then
  python3 generatePseudoIAM.py save_weights_smart Benchmark/research-seq2seq-HTR/dataset/IAM_smart --smart_patch_loss
fi
cd Benchmark/research-seq2seq-HTR/
python3 main_torch_latest.py -1 HTR_smart_weights IAM_smart
