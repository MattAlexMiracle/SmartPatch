...
class Decoder(nn.Module):
    def __init__(
        self, hidden_size, embedding_size, vocab_size, attention, tradeoff_context_embed
    ):
        ...
        if self.tradeoff is not None:
            self.context_shrink = nn.Linear(
                self.hidden_size, self.embed_size * self.tradeoff
            )  # !! trade-off between embedding and context
            self.gru = nn.GRU(
                self.embed_size * self.tradeoff + self.embed_size,
                self.hidden_size,
                self.n_layers,
    ...
    def forward(self, in_char, hidden, encoder_output, src_len, prev_attn):
        ...
        attn_weights = self.attention(
            hidden, encoder_output, enc_len, prev_attn
        )  # b, t, 1

        encoder_output_b = encoder_output.permute(1, 2, 0)  # b, f, t
        # print("attention", attn_weights.shape, "encoder", encoder_output_b.shape)
